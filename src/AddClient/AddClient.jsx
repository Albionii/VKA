...
export default function AddClient() {
  ...
  const handleSubmit = (event) => {
    ...
    const files = Array.from(formData.getAll("fotot"));

    // Log the files for debugging
    console.log("Files selected:", files);

    // Read files as Blob
    const readFilesAsBlobs = (files) => {
      return Promise.all(
        files.map(file => {
    ...
    readFilesAsBlobs(files).then(photos => {
      // Ensure that files have been read and stored as Blob
